...
def process_cell(distance_array, hic_array,random_index_list, start1_idx, end1_idx, start2_idx, end2_idx, start3_idx, end3_idx):
    ...
    return mean_distance, max_distance, circumcircle_distance, mean_distance_random, max_distance_random, circumcircle_distance_random,hic,hic_random
...
def process_chr(chr, df_true_chr, chr_idx_list):
    ...
    with h5py.File(f'distance_cells_{chr}.h5', 'r') as f:
        ...
        for i in range(len(df_true_chr)):
            ...
            mean_distance_list, max_distance_list, circumcircle_distance_list, mean_distance_list_random, max_distance_list_random, circumcircle_distance_list_random,hic_list,hic_list_random = zip(*results)
...
df_all_hic = pd.read_csv("trans_hic.csv")
df_all_hic['start1_idx'] = df_all_hic['start1'] // resolution
df_all_hic['end1_idx'] = df_all_hic['end1'] // resolution
df_all_hic['start2_idx'] = df_all_hic['start2'] // resolution
df_all_hic['end2_idx'] = df_all_hic['end2'] // resolution
df_all_hic['start3_idx'] = df_all_hic['start3'] // resolution
df_all_hic['end3_idx'] = df_all_hic['end3'] // resolution


df_all_hic = df_all_hic.head(234)
chr_list = df_all_hic['chr'].unique()
print(df_all_hic.head())


results = Parallel(n_jobs=20)(delayed(process_chr)(chr, df_all_hic.query("chr==@chr"), np.arange(0, bin_table_df.query("chrom==@chr").shape[0])) for chr in chr_list)
...
# Save the results
with open("all_hic_spatial_distance_mean_list.pickle", "wb") as f:
    # print(f"all_gam_spatial_distance_mean_list:{spatial_distance_mean_list}")
    pickle.dump(spatial_distance_mean_list, f)
with open("all_hic_spatial_distance_max_list.pickle", "wb") as f:
    # print(f"all_gam_spatial_distance_max_list:{spatial_distance_max_list}")
    pickle.dump(spatial_distance_max_list, f)
with open("all_hic_spatial_distance_circle_list.pickle", "wb") as f:
    # print(f"gam_spatial_distance_circle_list:{spatial_distance_circle_list}")
    pickle.dump(spatial_distance_circle_list, f)
with open("all_hic_spatial_distance_mean_random_bg_list.pickle", "wb") as f:
    # print(f"gam_spatial_distance_mean_random_bg_list:{spatial_distance_mean_random_bg_list}")
    pickle.dump(spatial_distance_mean_random_bg_list, f)
with open("all_hic_spatial_distance_max_random_bg_list.pickle", "wb") as f:
    # print(f"gam_spatial_distance_max_random_bg_list:{spatial_distance_max_random_bg_list}")
    pickle.dump(spatial_distance_max_random_bg_list, f)
with open("all_hic_spatial_distance_circle_random_bg_list.pickle", "wb") as f:
    # print(f"gam_spatial_distance_circle_random_bg_list:{spatial_distance_circle_random_bg_list}")
    pickle.dump(spatial_distance_circle_random_bg_list, f)

with open("all_hic_hic_list.pickle", "wb") as f:
    # print(f"gam_hic_list:{contact_hic_list}")
    pickle.dump(contact_hic_list, f)
with open("all_hic_hic_random_bg_list.pickle", "wb") as f:
    # print(f"gam_hic_random_bg_list:{contact_hic_random_bg_list}")
